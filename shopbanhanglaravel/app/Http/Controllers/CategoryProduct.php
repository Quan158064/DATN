...
class CategoryProduct extends Controller
{
    ...
    public function all_category_product(){
        ...
        $all_category_product = DB::table('tbl_category_product')->orderBy('category_id','DESC')->orderBy('category_order','ASC')->paginate(10);
       ...
       //slide
        ...
        if(isset($_GET['sort_by'])){
            ...
            if($sort_by=='giam_dan'){
                ...
                $category_by_id = $query->orderBy('product_price','DESC')->paginate(6)->appends(request()->query());

            }elseif($sort_by=='tang_dan'){

              $category_by_id = $query->orderBy('product_price','ASC')->paginate(6)->appends(request()->query());

          }elseif($sort_by=='kytu_za'){

           $category_by_id = $query->orderBy('product_name','DESC')->paginate(6)->appends(request()->query());


       }elseif($sort_by=='kytu_az'){

        $category_by_id = $query->orderBy('product_name','ASC')->paginate(6)->appends(request()->query());
    }

}elseif(isset($_GET['start_price']) && $_GET['end_price']){
    $min_price = $_GET['start_price'];
    $max_price = $_GET['end_price'];
    $category_by_id = $query->whereBetween('product_price',[$min_price,$max_price])->paginate(6);
}else{
    $category_by_id = $query->orderBy('product_id','DESC')->paginate(6);
}
